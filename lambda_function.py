...
def lambda_handler(event, context):
    ...
    if chat['id'] == int(ADMINCHAT):
        ...
        if command == '/Start_transport_EC2':
            ...
            ec2_bdh_usw2.start_instances(InstanceIds=instances_bdh_usw2)
            ec2_bdh_use1.start_instances(InstanceIds=instances_bdh_use1)
            ec2_bdh_euc1.start_instances(InstanceIds=instances_bdh_euc1)
            bot.send_message(chat['id'], 'BDH instances are going to be started.')
            return RESPONSE_200

        elif command == '/Stop_BDH_EC2':
            ec2_bdh_usw2.stop_instances(InstanceIds=instances_bdh_usw2)
            ec2_bdh_use1.stop_instances(InstanceIds=instances_bdh_use1)
            ec2_bdh_euc1.stop_instances(InstanceIds=instances_bdh_euc1)
            bot.send_message(chat['id'], 'BDH instances are going to be stopped.')
            return RESPONSE_200
